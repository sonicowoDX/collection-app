...
export default function App() {
  ...
  async function handleUploadCSV(e) {
    ...
    Papa.parse(file, {
      ...
      complete: async (results) => {
        ...
        // Crear juegos con imagenes usando Promise.all
        const gamesData = await Promise.all(rows.map(async (r) => {
          ...
          // Aquí sí esperamos la URL de la imagen
  ...
  async function getBGGImage(objectid) {
    try {
      const res = await fetch(`https://www.boardgamegeek.com/xmlapi2/thing?id=${objectid}`);
      const text = await res.text();
      const parser = new DOMParser();
      const xml = parser.parseFromString(text, "text/xml");
      const image = xml.querySelector("image")?.textContent;
      return image || "ruta/a/imagen/default.png"; // fallback si no hay imagen
    } catch (err) {
  ...
  if (!isUserConfirmed) {
    return (
      <div style={{ textAlign: "center", marginTop: "50px" }}>
        ...
        <button
          onClick={() => {
            if (username.trim() !== "") setIsUserConfirmed(true);
          }}
        >
          Confirmar
        </button>
  ...
  return (
    <div style={{ fontFamily: "Arial, sans-serif", padding: "10px", textAlign: "center" }}>
      ...
      <div style={{ margin: "20px 0", textAlign: "center" }}>
        <h2>Subir colección (Colección exportada de la BGG)</h2>
        <input
          type="file"
          accept=".csv"
